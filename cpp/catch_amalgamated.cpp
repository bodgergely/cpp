...
sample resample(URng& rng, int resamples, std::vector<double>::iterator first,
                std::vector<double>::iterator last, Estimator& estimator)
{
    auto n = last - first;
...
public:  // IMutableRegistryHub
    void registerReporter(std::string const& name,
                          IReporterFactoryPtr factory) override
    {
    ...
    {
        m_testCaseRegistry.registerTest(std::move(testInfo),
                                        std::move(invoker));
    }
          ...
          m_context{config, std::move(reporter)}
    {
        ...
        if (m_matches.empty() && invalidArgs.empty()) {
            for (auto const& test : allTestCases)
                if (!test.getTestCaseInfo().isHidden()) m_tests.emplace(&test);
        }
        else {
...
int Session::applyCommandLine(int argc, wchar_t const* const* argv)
{
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, utf8Argv[i], bufSize,
                            nullptr, nullptr);
    }
...
{
    if (!m_config) m_config = Detail::make_unique<Config>(m_configData);
    return *m_config;
}
...
int Session::runInternal()
{
    ...
    CATCH_TRY
    {
        ...
        // Set up global config instance before we start calling into other
        // functions
        getCurrentMutableContext().setConfig(m_config.get());
...
static_assert(
    sizeof(TestCaseProperties) == sizeof(TCP_underlying_type),
    "The size of the TestCaseProperties is different from the assumed size");

TestCaseProperties operator|(TestCaseProperties lhs, TestCaseProperties rhs)
{
    return static_cast<TestCaseProperties>(
        static_cast<TCP_underlying_type>(lhs) |
...
{
    lhs =
        static_cast<TestCaseProperties>(static_cast<TCP_underlying_type>(lhs) |
                                        static_cast<TCP_underlying_type>(rhs));
    return lhs;
}
...
{
    static_assert(
        static_cast<TCP_underlying_type>(TestCaseProperties::None) == 0,
...
TestCaseProperties parseSpecialTag(StringRef tag)
{
    if (!tag.empty() && tag[0] == '.')
        ...
        return static_cast<TestCaseProperties>(TestCaseProperties::Benchmark |
...
StringRef extractFilenamePart(StringRef filename)
{
    ...
    while (nameStart > 0 && filename[nameStart - 1] != '/' &&
...
Detail::unique_ptr<TestCaseInfo> makeTestCaseInfo(
    std::string const& _className, NameAndTags const& nameAndTags,
    SourceLineInfo const& _lineInfo)
{
    return Detail::unique_ptr<TestCaseInfo>(
        new TestCaseInfo(_className, nameAndTags, _lineInfo));
}
      ...
      lineInfo(_lineInfo)
{
    ...
    for (size_t idx = 0; idx < originalTags.size(); ++idx) {
        ...
        if (c == ']') {
            ...
            StringRef tagStr =
                originalTags.substr(tagStart + 1, tagEnd - tagStart - 1);
    ...
    std::sort(begin(tags), end(tags),
              [](Tag lhs, Tag rhs) { return lhs.lowerCased < rhs.lowerCased; });
...
void TestCaseInfo::internalAppendTag(StringRef tagStr)
{
    ...
    backingLCaseTags += ']';
    tags.emplace_back(StringRef(backingTags.c_str() + backingStart,
                                backingEnd - backingStart),
                      StringRef(backingLCaseTags.c_str() + backingStart,
                                backingEnd - backingStart));
}

bool TestCaseHandle::operator==(TestCaseHandle const& rhs) const
{
    return m_invoker == rhs.m_invoker && m_info->name == rhs.m_info->name &&
...
TestCaseInfo const& TestCaseHandle::getTestCaseInfo() const { return *m_info; }
...
{
    return std::find_if(begin(testCase.tags), end(testCase.tags),
                        [&](Tag const& tag) {
...
TestSpec::Matches TestSpec::matchesByFilter(
    std::vector<TestCaseHandle> const& testCases, IConfig const& config) const
{
...
void IStreamingReporter::listReporters(
    std::vector<ReporterDescription> const& descriptions, IConfig const& config)
{
    Catch::cout() << "Available reporters:\n";
    const auto maxNameLen =
        std::max_element(
            descriptions.begin(), descriptions.end(),
            [](ReporterDescription const& lhs, ReporterDescription const& rhs) {
    ...
    for (auto const& desc : descriptions) {
        if (config.verbosity() == Verbosity::Quiet) {
            Catch::cout() << TextFlow::Column(desc.name).indent(2).width(
                                 5 + maxNameLen)
                          << '\n';
        }
        else {
            Catch::cout() << TextFlow::Column(desc.name + ":")
                                     .indent(2)
                                     .width(5 + maxNameLen) +
                                   ...
                                   IConfig const& config)
{
    ...
    for (auto const& test : tests) {
        ...
        Catch::cout()
            << TextFlow::Column(testCaseInfo.name).initialIndent(2).indent(4)
            << '\n';
        if (config.verbosity() >= Verbosity::High) {
            Catch::cout() << TextFlow::Column(Catch::Detail::stringify(
        ...
        if (!testCaseInfo.tags.empty() &&
            ...
            Catch::cout()
                << TextFlow::Column(testCaseInfo.tagsAsString()).indent(6)
                << '\n';
        }
    }
                                       ...
                                       Detail::TokenStream const& tokens) const
{
    ...
    if (!result)
        ...
        return Detail::InternalParseResult::ok(
            Detail::ParseState(ParseResultType::Matched, ++remainingTokens));
}
        ...
        if (isMatch(token.token)) {
            if (m_ref->isFlag()) {
                ...
                if (result.value() == ParseResultType::ShortCircuitAll)
                    return Detail::InternalParseResult::ok(
                        Detail::ParseState(result.value(), remainingTokens));
            }
            else {
                auto valueRef =
...
Detail::InternalParseResult ExeName::parse(
    std::string const&, Detail::TokenStream const& tokens) const
{
    return Detail::InternalParseResult::ok(
...
Detail::InternalParseResult Parser::parse(
    std::string const& exeName, Detail::TokenStream const& tokens) const
{
    struct ParserInfo {
...
#else  // ^^ Windows crt debug heap enabled // Windows crt debug heap disabled
       // vv
...
private:
    void setColour(const char* _escapeCode)
    {
        ...
        // stdin and stderr are intermixed, we'd get accidentally coloured
        // output.
...
Catch::Detail::unique_ptr<EnumInfo> makeEnumInfo(StringRef enumName,
    ...
    const IExceptionTranslator* translator)
{
    m_translators.push_back(
        Detail::unique_ptr<const IExceptionTranslator>(translator));
}
...
LONG CALLBACK FatalConditionHandler::handleVectoredException(
...
{
    if (isSet) {
        // Set signals back to previous values -- hopefully nobody overwrote
...
{
    auto const& testSpec = config.testSpec();
    std::vector<TestCaseHandle> matchedTestCases =
        filterTests(getAllTestCasesSorted(config), testSpec, config);
    ...
    for (auto const& testCase : matchedTestCases) {
        for (auto const& tagName : testCase.getTestCaseInfo().tags) {
            ...
            if (it == tagCounts.end())
                it = tagCounts
                         .insert(std::make_pair(tagName.lowerCased, TagInfo()))
                         .first;
...
struct StringStreams {
    ...
    auto add() -> std::size_t
    {
        if (m_unused.empty()) {
            m_streams.push_back(
                Detail::unique_ptr<std::ostringstream>(new std::ostringstream));
...
std::vector<TestCaseHandle> sortTests(
    IConfig const& config, std::vector<TestCaseHandle> const& unsortedTestCases)
{
...
bool matchTest(TestCaseHandle const& testCase, TestSpec const& testSpec,
               IConfig const& config)
{
...
void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const& functions)
{
    ...
    for (auto const& function : functions) {
        ...
        CATCH_ENFORCE(
            prev.second,
            "error: TEST_CASE( \""
                << function.getTestCaseInfo().name << "\" ) already defined.\n"
...
std::vector<TestCaseHandle> filterTests(
    std::vector<TestCaseHandle> const& testCases, TestSpec const& testSpec,
    IConfig const& config)
{
...
std::vector<TestCaseHandle> const& TestRegistry::getAllTestsSorted(
    IConfig const& config) const
{
    if (m_sortedFunctions.empty()) enforceNoDuplicateTestCases(m_handles);
...
ITracker& TrackerContext::startRun()
{
    ...
    m_rootTracker = std::make_shared<SectionTracker>(
        NameAndLocation("{root}"s, CATCH_INTERNAL_LINEINFO), *this, nullptr);
...
{
    return Detail::unique_ptr<ITestInvoker>(
        new TestInvokerAsFunction(testAsFunction));
}

AutoReg::AutoReg(Detail::unique_ptr<ITestInvoker> invoker,
                 SourceLineInfo const& lineInfo, StringRef const& classOrMethod,
                 NameAndTags const& nameAndTags) noexcept
{
    CATCH_TRY
    {
        getMutableRegistryHub().registerTest(
            makeTestCaseInfo(extractClassName(classOrMethod), nameAndTags,
                             lineInfo),
            std::move(invoker));
    }
    CATCH_CATCH_ALL
...
{
    if (!m_currentFilter.m_required.empty() ||
        !m_currentFilter.m_forbidden.empty()) {
...
void TestSpecParser::addTagPattern()
{
    ...
    if (!token.empty()) {
        ...
        if (token.size() > 1 && token[0] == '.') {
            ...
            if (m_exclusion) {
                m_currentFilter.m_forbidden.emplace_back(
                    Detail::make_unique<TestSpec::TagPattern>(".",
                                                              m_substring));
                m_currentFilter.m_forbidden.emplace_back(
                    Detail::make_unique<TestSpec::TagPattern>(token,
                                                              m_substring));
            }
            else {
                m_currentFilter.m_required.emplace_back(
...
{
    auto initial = m_pos == 0 ? m_column.m_initialIndent : std::string::npos;
    return initial == std::string::npos ? m_column.m_indent : initial;
}

std::string Column::iterator::addIndentAndSuffix(size_t position,
                                                 size_t length) const
{
    std::string ret;
...
{
    return !!(static_cast<std::underlying_type_t<XmlFormatting>>(
...
{
    return WithinUlpsMatcher(target, maxUlpDiff,
...
{
    return WithinUlpsMatcher(target, maxUlpDiff,
                             Detail::FloatingPointKind::Float);
}
...
{
    return WithinRelMatcher(target,
...
{
    return m_caseSensitivity == CaseSensitive::Yes ? StringRef()
...
StringEqualsMatcher::StringEqualsMatcher(CasedString const& comparator)
...
StringContainsMatcher::StringContainsMatcher(CasedString const& comparator)
...
RegexMatcher::RegexMatcher(std::string regex, CaseSensitive caseSensitivity)
...
std::string RegexMatcher::describe() const
{
    return "matches " + ::Catch::Detail::stringify(m_regex) +
           ((m_caseSensitivity == CaseSensitive::Yes) ? " case sensitively"
                                                      : " case insensitively");
}

StringEqualsMatcher Equals(std::string const& str,
                           CaseSensitive caseSensitivity)
{
    return StringEqualsMatcher(CasedString(str, caseSensitivity));
}
StringContainsMatcher Contains(std::string const& str,
                               CaseSensitive caseSensitivity)
{
...
StartsWithMatcher StartsWith(std::string const& str,
                             CaseSensitive caseSensitivity)
{
...
std::string describe_multi_matcher(StringRef combine,
                                   ...
                                   std::string const* descriptions_end)
{
    ...
    combined_size +=
        (descriptions_end - descriptions_begin - 1) * combine.size();
                              ...
                              StringRef const& matcherString)
{
    ...
    MatchExpr<std::string, StringMatcher const&> expr(
        std::move(exceptionMessage), matcher, matcherString);
...
std::string IsEmptyMatcher::describe() const { return "is empty"; }
...
IsEmptyMatcher IsEmpty() { return {}; }

HasSizeMatcher SizeIs(std::size_t sz) { return HasSizeMatcher{sz}; }
...
std::string Catch::Matchers::Detail::finalizeDescription(
...
bool EventListenerBase::assertionEnded(AssertionStats const&) { return false; }
        ...
        if (!m_isOpen) {
            ...
            *this << RowBreak();
            ...
            for (auto const& info : m_columnInfos) {
                headerCols += TextFlow::Column(info.name).width(
...
void ConsoleReporter::benchmarkPreparing(std::string const& name)
{
    ...
    auto nameCol = TextFlow::Column(name).width(
...
struct BySectionInfo {
    ...
    bool operator()(
        std::shared_ptr<CumulativeReporterBase::SectionNode> const& node) const
    {
...
{
    auto it = std::find_if(begin(tags), end(tags), [](Tag const& tag) {
...
void ListeningReporter::listReporters(
    std::vector<ReporterDescription> const& descriptions, IConfig const& config)
{
    for (auto const& listener : m_listeners) {
...
void SonarQubeReporter::writeGroup(TestGroupNode const& groupNode)
{
    ...
    for (auto const& kv : testsPerFile) writeTestFile(kv.first, kv.second);
}

void SonarQubeReporter::writeTestFile(
    std::string const& filename, TestGroupNode::ChildNodes const& testCaseNodes)
{
    XmlWriter::ScopedElement e = xml.scopedElement("file");
    xml.writeAttribute("path", filename);

    for (auto const& child : testCaseNodes) writeTestCase(*child);
}
...
void SonarQubeReporter::writeSection(std::string const& rootName,
                                     SectionNode const& sectionNode,
                                     bool okToFail)
{
    std::string name = trim(sectionNode.stats.sectionInfo.name);
...
void SonarQubeReporter::writeAssertion(AssertionStats const& stats,
                                       bool okToFail)
{
    AssertionResult const& result = stats.assertionResult;
    if (!result.isOk()) {
...
public:
    ...
    TapAssertionPrinter(std::ostream& _stream, AssertionStats const& _stats,
                        std::size_t _counter)
        : stream(_stream),
          result(_stats.assertionResult),
          messages(_stats.infoMessages),
...
private:
    ...
    void printIssue(StringRef issue) const { stream << ' ' << issue; }
...
// if string has a : in first line will set indent to follow it on
// subsequent lines
void printHeaderString(std::ostream& os, std::string const& _string,
                       std::size_t indent = 0)
{
    std::size_t i = _string.find(": ");
    if (i != std::string::npos)
...
bool TeamCityReporter::assertionEnded(AssertionStats const& assertionStats)
{
    ...
    if (!result.isOk()) {
        ...
        if (!m_headerPrintedForThisSection) printSectionHeader(msg.get());
        ...
        if (assertionStats.infoMessages.size() == 1) msg << " with message:";
        if (assertionStats.infoMessages.size() > 1) msg << " with messages:";
...
void TeamCityReporter::testCaseEnded(TestCaseStats const& testCaseStats)
{
    ...
    if (!testCaseStats.stdOut.empty())
        stream << "##teamcity[testStdOut name='" << escape(testCaseInfo.name)
               << "' out='" << escape(testCaseStats.stdOut) << "']\n";
    if (!testCaseStats.stdErr.empty())
        stream << "##teamcity[testStdErr name='" << escape(testCaseInfo.name)
               << "' out='" << escape(testCaseStats.stdErr) << "']\n";
    stream << "##teamcity[testFinished name='" << escape(testCaseInfo.name)
...
void TeamCityReporter::printSectionHeader(std::ostream& os)
{
    ...
    if (m_sectionStack.size() > 1) {
        ...
        std::vector<SectionInfo>::const_iterator
            ...
            itEnd = m_sectionStack.end();
        for (; it != itEnd; ++it) printHeaderString(os, it->name);
        os << lineOfChars('-') << '\n';
    }
...
void XmlReporter::listReporters(
    std::vector<ReporterDescription> const& descriptions, IConfig const&)
{
                            ...
                            IConfig const&)
{
    ...
    for (auto const& test : tests) {
        ...
        m_xml.startElement("Line", XmlFormatting::Indent)
            .writeText(std::to_string(testInfo.lineInfo.line),
                       XmlFormatting::None)
            .endElement(XmlFormatting::Newline);
    }
}
